...
const lowLayer = {
    img: new Image(),
    speed: 0.3,
    yOffset: canvas.height - canvas.height * 0.15 // Настройка позиции чуть ниже уровня дороги
};
...
const roadLayer = {
    img: new Image(),
    speed: 0.5,
    yOffset: lowLayer.yOffset - canvas.height * 0.1 // Позиция так, чтобы дорога немного перекрывала низ
};
...
function drawLowLayer() {
    ...
    const layerHeight = canvas.height * 0.15; // Высота нижнего слоя
...
function drawRoadLayer() {
    ...
    const layerHeight = canvas.height * 0.1; // Высота слоя дороги
    const layerWidth = aspectRatio * layerHeight;
